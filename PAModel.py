...
class HardAndNetwork(nn.Module):
    def __init__(self, model1: PAModel, model2: PAModel, var_order: list[str]):
        super().__init__()
        self.model1 = model1.model
        self.model2 = model2.model
    ...
    def predict(self, inputs):
        with torch.no_grad():
            x = torch.tensor(inputs, dtype=torch.float32).unsqueeze(0).to(next(self.model1.parameters()).device)
            return self.forward(x).item() > 0.5


def combine_and_model(M1: PAModel, M2: PAModel, var_order: list[str], num_samples=100, use_hard_and=False) -> PAModel:
    z3_formula = And(M1.z3_formula, M2.z3_formula)
    combined_formula_str = f"And({M1.formula_str}, {M2.formula_str})"

    model_class = HardAndNetwork if use_hard_and else AndNetwork
    model = model_class(M1, M2, var_order).to(next(M1.model.parameters()).device)

    z3_vars = {v: Int(v) for v in var_order}
    pos, neg = [], []
    while len(pos) + len(neg) < num_samples:
        sample = [random.randint(0, 10) for _ in var_order]
        s = Solver()
        s.add(z3_formula)
